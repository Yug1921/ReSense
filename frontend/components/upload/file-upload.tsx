...
import { uploadPaper, ApiError, type UploadResponse } from "@/lib/api";
...
interface FileUploadProps {
    onUploadSuccess?: (file: File, response: UploadResponse) => void;
...
const DEFAULT_MAX_FILE_SIZE = 20 * 1024 * 1024; // 20MB — matches backend MAX_UPLOAD_MB
...
const formatAcceptedTypes = (types: string[]): string => {
    const labels = types.map((t) => MIME_TO_LABEL[t] ?? t.split("/")[1]?.toUpperCase() ?? t);
...
export default function FileUpload({
    ...
    const validateFileType = useCallback(
        (file: File): FileError | null => {
            ...
            if (
                ...
                return {
                    message: `File type must be ${formatAcceptedTypes(acceptedFileTypes)}`,
    ...
    const uploadFile = useCallback(
        (uploadingFile: File) => {
            const { promise, abort } = uploadPaper(uploadingFile, (percent) => {
            ...
            promise
                .then((response) => {
                    ...
                    setStatus("idle");
                    setFile(null);
                    onUploadSuccess?.(uploadingFile, response);
                })
                .catch((err: unknown) => {
                    const message =
                    ...
                    handleError({ message, code: "UPLOAD_FAILED" });
                });
        },
        [onUploadSuccess]
    );
